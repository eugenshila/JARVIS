...
function defaultBridgeUrl(): string {
  // Server-side (tests, SSR): nothing to derive from, so keep the classic default.
  if (typeof window === 'undefined') return 'ws://localhost:8787'
  const { protocol, hostname, host } = window.location
  // The normal case — the bridge is on this machine, reached directly.
  if (hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '[::1]') {
    return 'ws://localhost:8787'
  }
  /**
   * Anywhere else, "localhost" would mean the *viewer's* machine — a phone on
   * the LAN, a remote sandbox, a tunnel — and the socket would connect to
   * nothing. The dev server proxies /bridge to the real one (see
   * vite.config.ts), so the page, the socket and the media endpoints all share
   * a single origin, which is also what the page's CSP already allows.
   */
  return `${protocol === 'https:' ? 'wss:' : 'ws:'}//${host}/bridge`
}

export const BRIDGE_WS_URL = str(import.meta.env.VITE_BRIDGE_URL) ?? defaultBridgeUrl()
