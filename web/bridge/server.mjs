...
import {
  ...
  JARVIS_AGENT,
  JARVIS_API_URL,
} from './jarvis-brain.mjs'
import { existsSync } from 'node:fs'
import { delimiter as PATH_SEP } from 'node:path'
 ...
 * Which brain answers.
 *
 * Upstream this is always the Claude Agent SDK, which needs a Claude Code
 * subscription. This fork also has a local one — the Python stack in
 * `src/jarvis`, served by `jarvis serve` — so the interface is useful on a
 * machine that has OpenAI keys, or Ollama, or nothing but the mock engine.
 *
 *   claude  Claude Code, headless (upstream)
 *   jarvis  our Python API at JARVIS_API_URL
 *   auto    Claude when it is installed and logged in, otherwise ours
 */
const BRAIN_CHOICE = (process.env.JARVIS_BRAIN ?? 'auto').toLowerCase()

/** Is the `claude` CLI on this machine and logged in? The SDK spawns it. */
function claudeAvailable() {
  if (existsSync(join(homedir(), '.claude.json'))) return true
  const dirs = (process.env.PATH ?? '').split(PATH_SEP).filter(Boolean)
  return dirs.some(
    (dir) => existsSync(join(dir, 'claude')) || existsSync(join(dir, 'claude.exe')),
  )
}

/** Settled before the server accepts sockets, so the banner can state it. */
let BRAIN = BRAIN_CHOICE === 'auto' ? 'claude' : BRAIN_CHOICE

async function resolveBrain() {
  if (BRAIN_CHOICE !== 'auto') return BRAIN_CHOICE
  if (claudeAvailable()) return 'claude'
  if (await jarvisApiReachable()) return 'jarvis'
  // Neither is ready. Prefer ours: it prints a usable instruction ("start it
  // with jarvis serve") instead of a login error from somebody else's CLI.
  return 'jarvis'
}
...
/**
 * Wildcard entries in JARVIS_ALLOWED_ORIGINS, e.g. `https://*.e2b.app`.
 *
 * Remote dev environments hand you a hostname you cannot know in advance (a
 * per-sandbox subdomain), so an exact list cannot be written down ahead of
 * time. A single `*` standing for one label keeps this far short of "allow
 * anything": the scheme and the registrable domain still have to match.
 */
function wildcardAllows(origin) {
  for (const entry of EXTRA_ORIGINS) {
...
function originAllowed(origin) {
  ...
  if (wildcardAllows(origin.replace(/\/+$/, ''))) return true
  let url
  try {
    url = new URL(origin)
  } catch {
  ...
  if (req.method === 'GET' && req.url === '/health') {
    ...
    return res.end(
      JSON.stringify({
        ...
        tts: eleven,
        stt: eleven,
        // Which brain is answering, so the HUD and `jarvis doctor` can say so
        // rather than leaving a silent assistant to be diagnosed by guesswork.
        brain: BRAIN,
        ...(BRAIN === 'jarvis' ? { api: JARVIS_API_URL, agent: JARVIS_AGENT } : { model: MODEL }),
      }),
    )
  }
...
// Decide the brain BEFORE the first socket can arrive: a connection that lands
// while the choice is still pending would be handed to whichever half happened
// to be the default, and the failure ("he hears me and never answers") gives no
// hint of a race.
BRAIN = await resolveBrain()
...
if (BRAIN === 'jarvis') {
  console.log(
    `[jarvis] brain SHILATECH python · ${JARVIS_API_URL} · agent ${JARVIS_AGENT}` +
      (await jarvisApiReachable()
        ? ''
        : ' — NOT REACHABLE yet, start it with: jarvis serve'),
  )
} else {
  console.log(`[jarvis] brain claude code · model ${MODEL} · effort ${EFFORT}`)
}
console.log(
...
wss.on('connection', (socket) => {
  ...
  // The SHILATECH path: same protocol, our Python brain. Everything below this
  // point is the Claude Agent SDK session, which this connection never opens.
  if (BRAIN === 'jarvis') {
    attachJarvisBrain(socket)
    return
  }
